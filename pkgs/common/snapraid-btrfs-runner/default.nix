{
  ...
  snapperConfigs ? "", # Comma-separated list of snapper configs (e.g., "data0,data1")
  deleteThreshold ? 40, # Abort if more than this many deletes
}:
let
  ...
  config = writeTextFile {
    ...
    text = ''
      ...
      ; Comma-separated list of snapper configs to snapshot before sync
      ; Configured via package parameters in storage config
      snapper-configs = ${snapperConfigs}
      ; Pool command not needed (we use mergerfs instead)
      pool = false
      ...
      deletethreshold = ${toString deleteThreshold}
