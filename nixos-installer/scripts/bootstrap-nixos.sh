...
function help_and_exit() {
	echo
	echo "Remotely installs NixOS on a target machine using yano nix-config."
	echo
	...
	echo '  --luks-secondary-drive-labels <drives>  specify the luks device names (as declared with "disko.devices.disk.*.content.luks.name" in disk configs) separated by commas.'
	echo '                                          Example: --luks-secondary-drive-labels "cryptdata1,cryptdata2"'
...
function luks_setup_secondary_drive_decryption() {
	...
	local key=/luks-secondary-unlock.key
...
if yes_or_no "Do you want to copy your full nix-config and nix-secrets to $target_hostname?"; then
	...
	if yes_or_no "Do you want to rebuild immediately?"; then
		...
		$ssh_cmd "cd ${nix_src_path}yano && sudo nixos-rebuild --impure --show-trace --flake .#$target_hostname switch"
	fi
else
	echo
	...
	echo "rsync -av --mkpath yano $target_user@$target_destination:${nix_src_path}"
	echo "To rebuild, sign into $target_hostname and run the following command"
	echo "cd ${nix_src_path}yano"
